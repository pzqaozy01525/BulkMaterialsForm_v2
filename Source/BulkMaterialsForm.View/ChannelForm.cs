...
// Type: BulkMaterialsForm.View.ChannelForm
...
public class ChannelForm : Form
{
	...
	private void barButtonItem2_ItemClick(object sender, ItemClickEventArgs e)
	{
		...
		if (num >= 0)
		{
			...
			ChannelSetForm channelSetForm = new ChannelSetForm();
			channelSetForm.type = 1;
			channelSetForm.id = id;
			channelSetForm.ShowDialog();
			InitData();
		}
	}
	...
	private void barButtonItem3_ItemClick(object sender, ItemClickEventArgs e)
	{
		...
		int id = Convert.ToInt32(dataGridView1.Rows[num].Cells["id"].Value);
		if (MessageBox.Show("确认删除该通道配置？", "确认删除", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
		{
			if (new DataServerContext<tb_Channel>().Current.Delete((tb_Channel it) => it.id == id))
			{
