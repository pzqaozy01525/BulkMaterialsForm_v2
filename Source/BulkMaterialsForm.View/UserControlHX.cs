...
// Type: BulkMaterialsForm.View.UserControlHX
...
public class UserControlHX : UserControl, IVideoMngInterface
{
	...
	public void Init()
	{
		...
		Operation += UserControlHX_Operation;
		onPlate = MainData.video_HuaXia_CALLBACK;
		onIOEvent = MainData.video_HuaXia_OnIOEvent;
	...
	private void UserControlHX_Operation(int msg, byte type)
	{
		openType = type;
		if (tb_Device.id == msg)
		{
	...
	public void videoClientOpen_HuaXia()
	{
		lock (QianYe_lock)
		{
			if (!(_logHandle != IntPtr.Zero))
			{
				try
				{
					_logHandle = ipcsdk.ICE_IPCSDK_OpenPreview(tb_Device.CameraIP, 1, 1, (uint)(int)videoHandle, onPlate, new IntPtr(0));
					if (_logHandle != IntPtr.Zero)
					{
						MainData.HtHuaXiaCallBackDel[tb_Device.CameraIP] = new MainData.SDK_OnPlate(SDK_OnPlate);
						MainData.HtHuaXiaCallBackDelIO[tb_Device.CameraIP] = new MainData.SDK_OnIOEvent(SDK_OnIOEvent);
						ipcsdk.ICE_IPCSDK_SetIOEventCallBack(_logHandle, onIOEvent, new IntPtr(0));
						setButtonEnable(btEnable: true);
					}
				}
				catch (Exception ex)
				{
					LogSave.HXLog(DateTime.Now.ToString() + "播放视频失败" + ex.ToString());
				}
			}
		}
		IsEnd = false;
	...
	public void SDK_OnIOEvent(IntPtr pvParam, [In][MarshalAs(UnmanagedType.LPStr)] string pcIP, uint u32EventType, uint u32IOData1, uint u32IOData2, uint u32IOData3, uint u32IOData4)
	{
		if (pcIP.Equals(tb_Device.CameraIP))
		{
			barrierGate((byte)u32IOData1);
		}
	}
	...
	private void btnOpen_Click(object sender, EventArgs e)
	{
		...
		else if (UserControlHX.Operation != null)
		{
			UserControlHX.Operation(Convert.ToInt32(tb_Device.OpeningMethod), 1);
		}
	}
	...
	public void Open()
	{
		ipcsdk.ICE_IPCSDK_OpenGate(_logHandle);
	}
	...
	public void SDK_OnPlate(IntPtr pvParam, [In][MarshalAs(UnmanagedType.LPStr)] string pcIP, [In][MarshalAs(UnmanagedType.LPStr)] string pcNumber, [In][MarshalAs(UnmanagedType.LPStr)] string pcColor, IntPtr pcPicData, uint u32PicLen, IntPtr pcCloseUpPicData, uint u32CloseUpPicLen, short nSpeed, short nVehicleType, short nReserved1, short nReserved2, float fPlateConfidence, uint u32VehicleColor, uint u32PlateType, uint u32VehicleDir, uint u32AlarmType, uint u32SerialNum, uint uCapTime, uint u32ResultHigh, uint u32ResultLow)
	{
		try
		{
			if (MainData.IsBecomeDue || !pcIP.Equals(tb_Device.CameraIP))
			{
				return;
			}
			string text;
			...
			if ((IntPtr)(long)(((ulong)u32ResultHigh << 32) + u32ResultLow) != IntPtr.Zero)
			{
				LogSave.HXLog(DateTime.Now.ToString() + pcNumber + "车牌开始颜色" + pcColor);
				showVehicleNo(pcNumber + "，平台验证中！");
				LedPrompt(pcNumber);
				text = "";
				...
				try
				{
					if (u32PicLen != 0)
					{
						byte[] array = new byte[u32PicLen];
						Marshal.Copy(pcPicData, array, 0, (int)u32PicLen);
						text = string.Format("{0}\\{1}.jpg", MainData.strImageDir, now.ToLocalTime().ToString("yyyyMMddHHmmssfff"));
						FileStream fileStream = new FileStream(text, FileMode.Create, FileAccess.ReadWrite, FileShare.None);
						fileStream.Write(array, 0, (int)u32PicLen);
						fileStream.Close();
						fileStream.Dispose();
					}
				}
				...
				{
					LogSave.HXLog(DateTime.Now.ToString() + "保存图片时异常" + ex.Message);
				}
				if (tb_Device.CameraType == "标准相机")
				{
					IsRelease = true;
					vehicleNoInfoView = new VehicleNoInfoView();
					vehicleNoInfoView.VehicleNo = pcNumber;
					...
					vehicleNoInfoView.ImagePath = text;
					vehicleNoInfoView.largeId = tb_Device.bigScreenId;
					vehicleNoInfoView.licenseColor = pcColor;
					...
					if (!isrg)
					{
						...
						if (array2 != null && array2.Length != 0 && !array2.Contains(pcColor))
						{
							vehicleNoInfoView.ExeLog = "此通道禁止此车牌颜色通行";
							vehicleNoInfoView.TrafficStatus = "禁止通行";
						}
						else
						{
							...
							if (array3 == null || array3.Length == 0 || !array3.Contains(pcColor) || new DataServerContext<tb_car_info>().Current.GetModel((tb_car_info it) => it.car_no == pcNumber) != null)
							{
								goto IL_037a;
							}
							vehicleNoInfoView.ExeLog = "此车牌号不属于白名单";
			...
			IL_0eed:
			isrg = false;
			LogSave.HXLog(DateTime.Now.ToString() + pcNumber + "车牌结束");
			return;
			IL_0491:
			if (MainData.dnyybb)
			{
				Task.Factory.StartNew(delegate
				{
					...
					if (IsRelease)
					{
						...
						pcNumber = vehicleNoInfoView.VehicleNo + vehicleNoInfoView.emissionStandard + vehicleNoInfoView.fuelType + text16;
						obj.Speak(pcNumber);
					}
					else
					{
						...
						pcNumber = vehicleNoInfoView.VehicleNo + text16;
						obj2.Speak(pcNumber);
					}
				});
			}
			...
			showVehicleNo(pcNumber);
			goto IL_0ed3;
			IL_037a:
			List<tb_car_info> list = new DataServerContext<tb_car_info>().Current.GetList((tb_car_info it) => it.car_no == pcNumber);
			if (list != null && list.Count > 0)
			{
				if (list[0].bz == "黑名单")
				{
			...
			if (MainData.DJPT == "消纳场")
			{
				...
				text2 = ((!(tb_Channel.ChannelType == "入口")) ? "2" : "1");
				string msg = "";
				if (CommonHelper.XNCVerify(pcNumber, pcColor, tb_Device.deviceId, text2, ref msg))
				{
					if (MainData.RecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, null))
					{
						IsRelease = true;
					}
					else
			...
			else if (MainData.DJPT == "高凌")
			{
				...
				string msg2 = "";
				bool isUpload = false;
				if (vehicleNoInfoView.licenseColor.Contains("黄绿"))
				{
					vehicleNoInfoView.licenseColor = "绿色";
				}
				if (!CommonHelper.GLVerify(pcNumber, vehicleNoInfoView.licenseColor, text3, ref msg2, ref vehicleNoInfoView))
				{
					isUpload = true;
					IsRelease = false;
					vehicleNoInfoView.ExeLog = msg2;
				}
				if (!MainData.RecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, vehicleNoInfoView, isUpload))
				{
					IsRelease = false;
					vehicleNoInfoView.ExeLog = msg2 + ";;保存失败";
				}
			...
			else if (MainData.DJPT == "腾跃")
			{
				...
				string msg3 = "";
				bool isJS = false;
				if (CommonHelper.TYVerify(pcNumber, pcColor, num, text, ref msg3))
				{
					isJS = true;
					IsRelease = true;
				}
				...
				if (!MainData.RecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, null, isUpload: false, "", isJS))
				{
					vehicleNoInfoView.ExeLog = "保存失败";
					IsRelease = false;
				}
			...
			else if (MainData.DJPT == "中科九州")
			{
				...
				string text5 = "";
				string text6 = "";
				if (CommonHelper.KangFengV1Verify(pcNumber, pcColor, tb_Channel.ChannelPort, text4, ref msg4, ref vehicleNoInfoView))
				{
					if (MainData.YZTZ)
					{
						if (CommonHelper.weiyouyuan(pcNumber, tb_Channel.ChannelType))
						{
							text6 = "未上传";
							text5 = "摆杆通行";
						}
				...
				if (!MainData.KaiFengRecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, vehicleNoInfoView.serialNum, text5, text6, ref vehicleNoInfoView))
				{
					vehicleNoInfoView.ExeLog = "保存失败";
					IsRelease = false;
				}
			...
			else if (MainData.DJPT == "安车")
			{
				...
				string text7 = "";
				string text8 = "";
				if (CommonHelper.AnCheVerify(pcNumber, pcColor, ref msg5, ref vehicleNoInfoView, tb_Channel.ChannelPort))
				{
					text8 = "未上传";
					text7 = "摆杆通行";
				}
				...
				text9 = ((!(tb_Channel.ChannelType == "入口")) ? "B" : "A");
				vehicleNoInfoView.serialNum = MainData.ACEnterpriseID + tb_Channel.ChannelPort + text9 + vehicleNoInfoView.AddTime.ToString("yyMMddHHmmss");
				if (!MainData.KaiFengRecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, vehicleNoInfoView.serialNum, text7, text8, ref vehicleNoInfoView))
				{
					vehicleNoInfoView.ExeLog = "保存失败";
					IsRelease = false;
				}
			...
			else if (MainData.DJPT == "天地人车")
			{
				...
				string text10 = "";
				string text11 = "";
				if (CommonHelper.tdrcCheVerify(pcNumber, pcColor, ref msg6, ref vehicleNoInfoView))
				{
					text11 = "未上传";
					text10 = "摆杆通行";
				}
				...
				string text12 = "A";
				text12 = ((!(tb_Channel.ChannelType == "入口")) ? "B" : "A");
				vehicleNoInfoView.serialNum = MainData.tdrcOrgan + tb_Channel.ChannelPort + text12 + DateTime.Now.ToString("yyMMddHHmmss");
				if (!MainData.KaiFengRecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, vehicleNoInfoView.serialNum, text10, text11, ref vehicleNoInfoView))
				{
					vehicleNoInfoView.ExeLog = "保存失败";
					IsRelease = false;
				}
			...
			else if (MainData.DJPT == "信阳")
			{
				...
				string text13 = "";
				string text14 = "";
				if (CommonHelper.xyCheVerify(pcNumber, pcColor, ref msg7, ref vehicleNoInfoView))
				{
					text14 = "未上传";
					text13 = "摆杆通行";
				}
				...
				text15 = ((!(tb_Channel.ChannelType == "入口")) ? "B" : "A");
				vehicleNoInfoView.serialNum = MainData.xyOrgan + tb_Channel.ChannelPort + text15 + DateTime.Now.ToString("yyMMddHHmmss");
				if (!MainData.KaiFengRecordSave(storePic(pcCloseUpPicData, u32CloseUpPicLen), text, pcNumber, pcColor, tb_Channel.ChannelType, tb_Channel.id, tb_Device.id, tb_Channel.ChannelPort, vehicleNoInfoView.serialNum, text13, text14, ref vehicleNoInfoView))
				{
					vehicleNoInfoView.ExeLog = "保存失败";
					IsRelease = false;
				}
		...
		{
			LogSave.HXLog(DateTime.Now.ToString() + "SDK_OnPlate回调报错:" + ex2.ToString());
		}
	}
	...
	public void LEDPlay(bool IsRelease, VehicleNoInfoView vehicleNoInfoView)
	{
		...
		if (!MainData.bxtxhw)
		{
			...
			{
				UserControlHX.Operation(Convert.ToInt32(tb_Device.OpeningMethod), 2);
			}
		}
		Led(vehicleNoInfoView.VehicleNo, isOpen: true);
		vehicleNoInfoView.TrafficStatus = "请通行";
		vehicleNoInfoView.ExeLog = "请通行";
	...
	{
		ipcsdk.ICE_IPCSDK_TransSerialPort(_logHandle, data, (uint)data.Length);
	}
	...
	public void Snap(string model)
	{
		...
		if (tb_Device.ChannelNo == Convert.ToInt32(dictionary["ChannelNo"]) && tb_Device.id != Convert.ToInt32(dictionary["DeviceId"]))
		{
			...
			DateTime now = DateTime.Now;
			string.Format("{0}\\{1}.jpg", MainData.strImageDir, now.ToLocalTime().ToString("yyyyMMddHHmmssfff"));
		}
	}
	...
	{
		StringBuilder pcNumber = new StringBuilder(32);
		StringBuilder pcColor = new StringBuilder(64);
		uint pu32PicLen = 0u;
		byte[] pcPicData = new byte[1048576];
		isrg = true;
		ipcsdk.ICE_IPCSDK_Trigger(_logHandle, pcNumber, pcColor, pcPicData, 1048576u, ref pu32PicLen);
	}
							...
							if (value > 0)
							{
								...
								Task.Factory.StartNew(delegate
								{
									...
									else
									{
										...
										LogSave.DHLog(DateTime.Now.ToString() + "DH开始录像intPtr=" + logHandle);
										DateTime now = DateTime.Now;
										Thread.Sleep(240000);
										new DHSDK().Download(now.AddMinutes(-1.0), now, value, Convert.ToInt32(zpjList.ChannelID), logHandle);
									}
								});
								if (zpjList != null)
								{
	...
	private void InitializeComponent()
	{
		...
		this.tableLayoutPanel1.Size = new System.Drawing.Size(644, 418);
		...
		this.tableLayoutPanel2.Size = new System.Drawing.Size(638, 34);
		...
		this.lalCarNo.Location = new System.Drawing.Point(490, 6);
		...
		this.gboxheader.Size = new System.Drawing.Size(638, 372);
		...
		this.pboxQY.Size = new System.Drawing.Size(632, 344);
		...
		base.Name = "UserControlHX";
		base.Size = new System.Drawing.Size(644, 418);
