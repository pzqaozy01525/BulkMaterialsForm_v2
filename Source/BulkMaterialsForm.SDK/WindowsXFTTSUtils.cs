...
// Type: BulkMaterialsForm.SDK.WindowsXFTTSUtils
...
public class WindowsXFTTSUtils
{
	...
	public static bool GetAudio(string text, string filename, string speaker, int speed)
	{
		...
		try
		{
			...
			p = QTTSSessionBegin("engine_type = local,rdn = 2, speed = 4, volume = 100, rcn = 0, voice_name=xiaoyan, tts_res_path =fo|res\\tts\\xiaoyan.jet;fo|res\\tts\\common.jet, sample_rate = 16000", ref errorCode);
			if (errorCode != 0)
			{
				return false;
			}
			...
			byte[] array2 = StructToBytes(GetWave_Header((int)memoryStream.Length - 44));
