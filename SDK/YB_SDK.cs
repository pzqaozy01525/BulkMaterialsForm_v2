...
public class YB_SDK
{
	...
	public static void YBLedSendMes(tb_large_screen tb_Large_Screen, VehicleNoInfoView vehicleNoInfoView, bool isOpen)
	{
		try
		{
			...
			uint num3 = LedConnect(tb_Large_Screen.IP);
			LogSave.YBLog("hand:" + num3);
			DataServerContext<tb_large_screen_detaile> dataServerContext = new DataServerContext<tb_large_screen_detaile>();
			List<tb_large_screen_detaile> list = dataServerContext.Current.GetList((tb_large_screen_detaile it) => it.largeId == tb_Large_Screen.id);
			if (list != null && list.Count > 0)
			{
				int tlsj = 0;
				foreach (tb_large_screen_detaile item in list)
				{
					if (item.customText.Contains("{车牌号}") || item.customText.Contains("{排放阶段}") || item.customText.Contains("{燃油类型}") || item.customText.Contains("{预警等级}") || item.customText.Contains("{管控策略}") || item.customText.Contains("{响应等级}") || item.customText.Contains("{通行状态}") || item.customText.Contains("{验证提示}"))
					{
						Led5kSDK.bx_5k_area_header bx_5k_area_header = default(Led5kSDK.bx_5k_area_header);
						tlsj = item.tlsj;
						bx_5k_area_header.AreaType = 0;
						bx_5k_area_header.DynamicAreaLoc = (byte)num2;
						...
						string text = item.customText.Replace("{车牌号}", vehicleNoInfoView.VehicleNo).Replace("{排放阶段}", vehicleNoInfoView.emissionStandard).Replace("{燃油类型}", vehicleNoInfoView.fuelType)
							.Replace("{预警等级}", MainData.GLcontrolLevel)
							.Replace("{管控策略}", MainData.GLcontrolStrategy)
							.Replace("{响应等级}", MainData.GLresponseLevel)
						...
						LogSave.YBLog("SCREEN_SendDynamicArea:" + Led5kSDK.SCREEN_SendDynamicArea(num3, bx_5k_area_header, (ushort)bx_5k_area_header.DataLen, bytes));
			...
			if (isOpen)
			{
				num = Led5kSDK.SCREEN_Send_one_IO(num3, 0, 1);
			}
			if (num3 >= 0)
			{
				Led5kSDK.Destroy(num3);
			}
		}
		catch (Exception ex)
		{
	...
	public static void YBLedGDSendMes(tb_large_screen tb_Large_Screen, VehicleNoInfoView vehicleNoInfoView, bool isOpen)
	{
		...
		uint num3 = LedConnect(tb_Large_Screen.IP);
		LogSave.YBLog("OFS_DeleteFile:" + Led5kSDK.OFS_DeleteFile(num3, 0, null));
		...
		if (list != null && list.Count > 0)
		{
			...
			foreach (tb_large_screen_detaile item in list)
			{
				if (!item.customText.Contains("{车牌号}") && !item.customText.Contains("{排放阶段}") && !item.customText.Contains("{燃油类型}") && !item.customText.Contains("{预警等级}") && !item.customText.Contains("{管控策略}") && !item.customText.Contains("{响应等级}") && !item.customText.Contains("{通行状态}") && !item.customText.Contains("{验证提示}"))
				{
					...
					tlsj = item.tlsj;
					header.AreaType = 0;
					header.DynamicAreaLoc = (byte)num2;
					...
					string text = "";
					text = ((!(MainData.GLcontrolStrategy == "")) ? MainData.GLcontrolStrategy : "暂无管控");
					...
					byte[] bytes = Encoding.Default.GetBytes(text2.Trim());
		...
		LogSave.YBLog("SendProgram:" + led5kProgram.SendProgram(num3));
		if (num3 >= 0)
		{
			Led5kSDK.Destroy(num3);
		}
	...
	private static void Timer1_Tick(object sender, EventArgs e)
	{
		...
		foreach (timerModel item in tb_Large_Screens)
		{
			...
			Led5kSDK.bx_5k_area_header bx_5k_area_header = default(Led5kSDK.bx_5k_area_header);
			DataServerContext<tb_large_screen_detaile> dataServerContext = new DataServerContext<tb_large_screen_detaile>();
			List<tb_large_screen_detaile> list2 = dataServerContext.Current.GetList((tb_large_screen_detaile it) => it.largeId == item.id);
